...
std::vector<ssd::Detection> nms(std::map<float, std::vector<ssd::Detection>> m, float nms_thresh)
{
...
std::vector<ssd::Detection> post_process_output(float *prob, float *locations, float conf_thresh, float nms_thresh)
{
  ...
  for (int i = 0; i < ssd::NUM_DETECTIONS; i++)
  {
    ...
    ssd::Detection det = {box, class_id, *conf};
